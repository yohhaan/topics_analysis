...
def chrome(domains, use_override=True):
    ...
    for domain in domains:
        ...
        topics = utils.check_override_list(processed_domain) if use_override else None
        if topics != None:
            # Domain is in override list
            for c in topics:
                print("{}\t".format(config.taxonomy[c]))
        else:
            # If not call the classifier
            topics = config.model.classify(processed_domain)
            ...
            for c in cats:
                print(
                    "{}\t{}\t{}".format(
                        c.score,
                        c.category_name,
                        config.taxonomy[int(c.category_name)],
                    )
                )
    print("\n")


def ml_model_top(domains, topT):
    """
...
def ml_model_chrome(domains):
    ...
    for domain in domains:
        ...
        if unknown_score and unknown_score / top_sum > config.min_unknown_score:
            print("{}\t{}\t{}".format(unknown_score, -2, config.taxonomy[-2]))
            print("\n")
            continue
        other = False
        for c in cats:
            if (
                ...
                and c.score >= config.min_topic_score
                and c.score / top_sum >= config.min_normalized_score_within_top_n
            ):
                other = True
                print(
                    "{}\t{}\t{}".format(
...
def chrome_csv(domains):
    ...
    for domain in domains:
        processed_domain = utils.process_domain(domain)
        topics = utils.check_override_list(processed_domain)
        if topics != None:
            if topics == []:
