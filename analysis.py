...
def crux_extract_sample_size_x(df, x, output_sample):
    """
    ...
    df_extract_temp = (
        ...
        .head(1)
        .sample(3 * x)
    )  # extract 3 times the sample size to have enough that are not in override list
...
def crux_augment_with_meta_description(output_sample, output_augmented):
    df = pd.read_json(output_sample, orient="records", lines=True)
...
def crux_verification(output_augmented, output_verified):
    """
    ...
    related, 1 = incorrect.
    """
    df = pd.read_json(output_augmented, orient="records", lines=True)
    ...
    for row in df.itertuples():
        ...
        with open(output_verified, "a") as f:
